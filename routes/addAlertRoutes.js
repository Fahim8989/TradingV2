...
router.post('/sendEmail/:id',authenticateToken, async (req, res) => {
  try {
    const alertId = req.params.id;
    const coinPair = req.params.coin_pair;
    const targetPrice = req.params.target_price;
    const userId = req.user.userId; 
    ...
    // Send email
    const mailOptions = {
      from: 'tradingpricer@gmail.com',
      to: 'fahimahmed0503@gmail.com',
      subject: `Alert: ${coinPair} crossed the target price of ${targetPrice}`,
